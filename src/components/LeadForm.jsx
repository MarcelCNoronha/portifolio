...
function LeadForm({ open, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
      ...
      <div role="dialog" aria-modal="true" aria-label="Formulário para baixar checklist" className="relative max-w-md w-full bg-metallic-purple-dark rounded-xl shadow-xl p-6 z-10">
        <button onClick={onClose} className="absolute top-3 right-3 text-gray-300 hover:text-white" aria-label="Fechar">✕</button>

        <h3 className="text-xl font-bold mb-2 text-gray-100">Baixe o Checklist: 10 passos para vender online</h3>
        <p className="text-sm mb-4 text-gray-300 text-justify">Deixe seu email e enviarei o checklist por email, além de abrir a versão para download imediatamente.</p>

        <form onSubmit={handleSubmit} className="space-y-3">
          <label className="block">
            <span className="text-sm text-gray-200">Nome</span>
            <input type="text" value={name} onChange={(e) => setName(e.target.value)} required className="mt-1 block w-full rounded-md border-gray-700 bg-metallic-purple-dark text-gray-100 px-3 py-2" />
          </label>

          <label className="block">
            <span className="text-sm text-gray-200">Email</span>
            <input type="email" value={email} onChange={(e) => setEmail(e.target.value)} required className="mt-1 block w-full rounded-md border-gray-700 bg-metallic-purple-dark text-gray-100 px-3 py-2" />
          </label>
